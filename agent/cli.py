...
from agent.cleanup import cleanup_reports, safe_root, validate_cleanup_request
from agent.env import detect_runtime_env, env_summary
from agent.memory import append_memory, compact_memory, read_recent_memory, sanitize_text
from agent.paths import discover_runs, discover_latest_run
from agent.runtime import AgentRuntimeGuard, prompt_hash, sync_guard_state
...
def agent_welcome(state: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "  list runs",
        "  set training device",
        "  set target pair_f1 >= 0.75 max_trials 3",
        "",
...
INTENT_RULES = [
    IntentRule("safe_smoke", ("smoke", "\u8fd0\u884c smoke", "run smoke")),
    IntentRule("safe_audit", ("clean audit", "\u8fd0\u884c audit", "run audit")),
    IntentRule("safe_compile", ("\u7f16\u8bd1 agent", "check llm.py", "py_compile")),
    IntentRule("cleanup", ("cleanup", "\u6e05\u7406", "\u4fdd\u7559"), parse_keep=True),
    IntentRule("show_memory", ("\u67e5\u770b\u8bb0\u5fc6", "memory", "history")),
    IntentRule("doctor", ("doctor", "\u7efc\u5408\u8bca\u65ad"), ("outputs/candidate", "config/candidate.yaml"), 2),
    IntentRule("inspect", ("inspect", "\u68c0\u67e5 candidate", "inspect candidate"), ("outputs/candidate",), 1),
    IntentRule("trace", ("trace", "\u8ffd\u8e2a"), path_limit=3),
    IntentRule("compare", ("compare", "\u5bf9\u6bd4"), ("outputs/candidate", "outputs/oldbase"), 2),
    IntentRule("case", ("case", "\u6837\u672c\u5206\u6790"), ("outputs/candidate/predictions.jsonl",), 1),
    IntentRule("diagnose", ("diagnose", "\u8bca\u65ad"), ("outputs/candidate",), 1),
    IntentRule("schedule", ("schedule", "\u8ba1\u5212", "\u4e0b\u4e00\u6b65"), ("outputs/candidate", "config/candidate.yaml"), 2),
    IntentRule("report", ("report", "\u62a5\u544a", "\u603b\u7ed3"), ("release/model_card.md", "release/results_summary.md", "release/limitations.md")),
    IntentRule("auditdata", ("auditdata", "\u6570\u636e\u5ba1\u8ba1"), ("dataset/archive/train.jsonl", "dataset/archive/test.jsonl")),
    ...
    IntentRule("set_target", ("\u8bbe\u5b9a\u76ee\u6807", "target pair", "target valid", "max_trials")),
    IntentRule("show_target", ("\u67e5\u770b\u8c03\u53c2\u76ee\u6807", "show target")),
    IntentRule("clear_target", ("\u6e05\u9664\u8c03\u53c2\u76ee\u6807", "clear target")),
    IntentRule("start_target_train", ("\u5f00\u59cb\u76ee\u6807\u8bad\u7ec3", "start target training")),
    IntentRule("benchmark_candidate", ("benchmark", "\u8dd1 benchmark")),
    IntentRule("train_candidate", ("train", "\u8bad\u7ec3")),
    IntentRule("show_env", ("\u67e5\u770b\u8fd0\u884c\u73af\u5883", "show env")),
    IntentRule("show_remote_login", ("\u67e5\u770b\u8fdc\u7a0b\u767b\u5f55\u547d\u4ee4", "remote login")),
    IntentRule("server_train_plan", ("\u670d\u52a1\u5668\u8bad\u7ec3", "server smoke")),
    IntentRule("local_dataset_check", ("\u68c0\u67e5\u672c\u5730\u6570\u636e\u96c6", "local dataset")),
    IntentRule("remote_dataset_check", ("\u670d\u52a1\u5668\u6570\u636e\u96c6", "remote dataset")),
    IntentRule("compare_datasets", ("\u5bf9\u6bd4\u6570\u636e\u96c6", "compare datasets")),
    IntentRule("sweep_decoding_plan", ("\u626b\u63cf\u89e3\u7801\u53c2\u6570", "sweep decoding")),
    IntentRule("audit_collator_plan", ("\u5ba1\u8ba1 collator", "audit collator")),
    IntentRule("trial_config_plan", ("\u751f\u6210 trial config", "trial config")),
]
...
def parse_target_spec(raw: str) -> tuple[bool, str]:
    ...
    trials = re.search(r"(?:鏈€澶氳皟鍙倈max(?:imum)?[_ ]?trials)\s*(\d+)", raw, re.IGNORECASE)
    if not match:
...
def execute_input(raw: str, state: dict[str, Any], agent: RNAAnalysisAgent, echo: bool = True) -> bool:
    ...
    try:
        ...
        if reason:
            ...
            concise_print(state, ["Select training target."], ["Where will training run? local or remote."], "Reply with local, remote, 浣跨敤 cuda, or 浣跨敤 cpu.")
            ...
            if ok:
                ...
                concise_print(state, ["Set tuning target."], [f"{spec['metric']} {spec['operator']} {spec['value']}", f"max_trials: {spec['max_trials']}"], "Use 寮€濮嬬洰鏍囪缁?candidate to generate a trial plan.")
            else:
                status = "blocked"
...
def run_agent_audit(args: argparse.Namespace) -> None:
    ...
    try:
        from agent.memory import compact_memory, read_recent_memory, sanitize_text
    ...
    # 7. cleanup safe_root
    try:
        from agent.cleanup import safe_root
    ...
    # 8. benchmark blocked
    try:
        from agent.safety import block_reason, DIAGNOSTIC_ARTIFACT_PATTERNS
    ...
    try:
        from agent.safety import CONFIRM_TRAIN
    ...
    # 10. discover_runs
    try:
        from agent.paths import discover_runs
